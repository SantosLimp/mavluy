...
export default function LoadingScreen({ onComplete, isReady = true, storeConfig }: LoadingScreenProps) {
  ...
  // Synchronously fetch cached color to eliminate any color flash on page reload
  ...
  // Cache latest primary color whenever it is defined
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!isReady) {
      // While data is still fetching, smoothly animate up to 88%
      timer = setInterval(() => {
        setProgress(prev => {
      ...
      // Data is ready! Quickly animate remaining progress to 100% and finish
      timer = setInterval(() => {
        setProgress(prev => {
          if (prev >= 100) {
            clearInterval(timer);
  ...
  return (
    ...
    >
      <div className="relative z-10 flex flex-col items-center text-center max-w-sm w-full">
        ...
        {/* Minimal Progress Bar with Right-Aligned Numbers */}
        <motion.div 
        ...
        >
          ...
          {/* Progress Percentage Displayed on the Right */}
